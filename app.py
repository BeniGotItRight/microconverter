...
CONVERSION_INFO = {
    "PDF → Word": {
        ...
        "use_cases": ["Contracts", "Manuscripts", "Legal Documents"],
        "tip": "Best for text-based PDFs. Scanned PDFs will be extracted as text/images.",
    },
    "PDF → Excel (Bank Statements)": {
        ...
        "use_cases": ["M-Pesa Statements", "Bank Statements", "Invoices"],
        "tip": "Falls back to row extraction if tables aren't detected.",
    },
    "Word → PDF": {
        ...
        "tip": "Preserves basic layout, fonts, and tables.",
    },
    "PowerPoint → PDF": {
        "description": "Convert PowerPoint presentations into high-quality PDF handouts.",
        "use_cases": ["Slide Sharing", "Study Material", "Archives"],
        "tip": "Extracts text and tables from every slide.",
    },
    "CSV → Excel": {
        "description": "Convert raw CSV data into clean, professionally formatted Excel workbooks.",
        "use_cases": ["Data Cleaning", "M-Pesa Reports", "System Exports"],
        "tip": "Auto-sizes columns and detects best encoding.",
    },
    "JSON → Excel": {
        "description": "Flatten complex, nested JSON data into a simple tabular Excel format.",
        "use_cases": ["API Exports", "Database Dumps", "Analysis"],
        "tip": "Automatically flattens nested objects.",
    },
}
...
<style>
    ...
    .hero-title { font-size: 3.5rem; font-weight: 800; color: white; letter-spacing: -0.04em; }
    ...
    .footer-text { font-size: 0.85rem; color: #64748b; margin-bottom: 0.25rem; }
...
with st.sidebar:
    st.markdown('<div style="font-size: 1.75rem; font-weight: 800; color: #10b981; margin-bottom: 1.5rem;">CONVERTEX</div>', unsafe_allow_html=True)
    
    # Section 1: Main Controls
    st.markdown('<div class="sb-title">1. Workflow Category</div>', unsafe_allow_html=True)
    category_list = list(CATEGORIES.keys())
    selected_cat = st.radio("Category", category_list, label_visibility="collapsed")
    
    st.markdown('<div class="sb-title">2. Select Operation</div>', unsafe_allow_html=True)
    ...
    # Section 2: App Capabilities
    st.markdown('<div class="sb-title">Platform Capabilities</div>', unsafe_allow_html=True)
    st.markdown(f"""
    <div class="sb-section">
        <div class="sb-feature">✅ 35+ Conversion Paths</div>
        <div class="sb-feature">✅ Advanced Table Extraction</div>
        <div class="sb-feature">✅ 100% Local Processing</div>
        <div class="sb-feature">✅ Mobile Responsive UI</div>
        <div class="sb-feature">✅ Multi-Encoding Support</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <div class="sb-section">
        <div class="sb-feature" style="color:#10b981;">● Engine: Operational</div>
        <div class="sb-feature">● Version: 2.0.4</div>
    </div>
    """, unsafe_allow_html=True)
...
<div class="hero-container">
    ...
    <div style="color:rgba(255,255,255,0.9); font-size:1.1rem; font-weight:500;">Premium Document & Data Processing Suite</div>
</div>
""", unsafe_allow_html=True)
...
with col2:
    info = CONVERSION_INFO.get(selected_label, {
        "description": "High-precision processing using the Convertex engine.",
        "use_cases": ["Business Reports", "Data Migration"],
        "tip": "Optimized for speed and accuracy.",
    })
    
    st.markdown(f"""
    <div class="info-card">
        <div style="font-size: 1.5rem; font-weight: 700; color: #10b981; margin-bottom: 0.75rem;">{selected_label}</div>
        <div style="color: #94a3b8; margin-bottom: 1rem;">{info['description']}</div>
        <div style="margin-bottom: 1.25rem;">
            {" ".join([f'<span class="use-case-tag">{u}</span>' for u in info.get('use_cases', [])])}
        </div>
        <div style="background: rgba(0,0,0,0.2); padding: 0.75rem; border-radius: 8px; font-size: 0.85rem; color: #cbd5e1; border-left: 3px solid #10b981;">
    ...
    uploaded_file = st.file_uploader(f"Choose file", type=allowed, label_visibility="collapsed")
    ...
    if uploaded_file:
        ...
        with st.expander("⚙️ Advanced Options", expanded=False):
            ...
            if file_ext == ".pdf" and target_ext == ".xlsx":
                pages_str = st.text_input("Pages (e.g. 0,1,2)", placeholder="All")
                if pages_str:
...
# Final Consolidated Footer Credits
st.markdown(f"""
<div class="footer-container">
    <div class="footer-brand">CONVERTEX</div>
    <div class="footer-text">Developed by <span class="footer-link">LexcoreTech</span></div>
    <div class="footer-text">Lead Architect: <span class="footer-link">Benson Motari</span></div>
